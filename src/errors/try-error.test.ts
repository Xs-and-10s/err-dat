...
suite("TryError", () => {
  test("1) with code: Nullish Value Caught -> creates an instance with the correct error code & cause & message", () => {
    const error = new TryError(
      TryError.CodeOptions.NULLISH_VALUE_CAUGHT,
      undefined,
    );

    expect(error.code).toBe(TryError.CodeOptions.NULLISH_VALUE_CAUGHT);
  ...
  test("2) with code: Not A Function -> creates an instance with the correct error code & cause & message", () => {
    const error = new TryError(
      TryError.CodeOptions.NOT_A_FUNCTION,
      "not a function",
    );

    expect(error.code).toBe(TryError.CodeOptions.NOT_A_FUNCTION);
  ...
  test("3) with code: Not An Array -> creates an instance with the correct error code & cause & message", () => {
    const error = new TryError(
      TryError.CodeOptions.NOT_AN_ARRAY,
      "not an array",
    );

    expect(error.code).toBe(TryError.CodeOptions.NOT_AN_ARRAY);
